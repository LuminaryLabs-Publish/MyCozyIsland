...
export function createCellPresentationProvider({ defineWorldEffectProvider, terrainSource, biomeStore, shorelineStore, vegetationStore, rockStore, propStore } = {}) {
  ...
  function build({ world, cell }) {
    ...
    const descriptor = createCellEffectDescriptor({
      ...
      data: {
        ...
        biomeHandleId: biome?.handleId ?? null,
        shorelineHandleId: shoreline?.handleId ?? null,
        vegetationHandleId: vegetation?.handleId ?? null,
        rockHandleId: rocks?.handleId ?? null,
        propHandleId: props?.handleId ?? null
      }
    });
    store.set(cell.id, { handleId, descriptor });
  ...
  return Object.freeze({
    provider: defineWorldEffectProvider({
      ...
      kind: "render-cell",
      phase: "presentation",
      requires: ["terrain-descriptor", "biome-weights", "shoreline-distance", "vegetation-instances", "rock-instances", "prop-instances"],
