...
export function createLegacyRenderSnapshotBridge({ composition, providerRuntime, getWorldSnapshot } = {}) {
  if (!composition || !providerRuntime || !getWorldSnapshot) throw new TypeError("createLegacyRenderSnapshotBridge requires composition, providerRuntime, and getWorldSnapshot.");
  function createSnapshot() {
    ...
    const rocks = rockInstances.length === composition.rocks.instances.length ? Object.freeze({ ...composition.rocks, instances: Object.freeze(rockInstances) }) : composition.rocks;
    const props = propObjects.length === composition.props.objects.length ? Object.freeze({ ...composition.props, objects: Object.freeze(propObjects) }) : composition.props;
