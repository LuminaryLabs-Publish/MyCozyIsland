...
const config = Object.freeze({
  ...
  terrain: Object.freeze({ resolution: 5 }),
...
let snapshot = {
  activeCells: [{ cell: far }, { cell: center }]
};
...
for (const cell of [center, far]) {
  terrainSource.prepareCell({ worldId: config.id, cell, seed: `${cell.id}:seed` });
...
assert.equal(terrainSamples, 0, "Core World registration must not materialize terrain.");
assert.equal(terrainSource.getRuntimeCell(center.id), null);
assert.equal(terrainSource.getRuntimeCell(far.id), null);
...
assert.equal(terrainSamples, 5, "One terrain row should be sampled in the first bounded step.");
assert.equal(terrainSource.getRuntimeCell(center.id), null, "A partial terrain job must not publish a ready runtime handle.");

let guard = 0;
while (materializer.getState().progress < 1 && guard < 200) {
  materializer.processFrame({
    focus: { x: 0, z: 0 },
    cameraMode: "first-person"
  });
  guard += 1;
...
assert.ok(guard < 200, "Lazy cell jobs should complete under the deterministic frame budget.");
...
assert.equal(
  presentation.runtimeStore.get(center.id).descriptor.materialization,
  "ready"
);
